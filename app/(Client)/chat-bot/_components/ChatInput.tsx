...
import { FileText, Globe, Mic, MicOff, Send, Settings, Share2, Upload } from 'lucide-react'
import React, { Dispatch, FormEvent, RefObject, SetStateAction, memo, useState } from 'react'
...
const ChatInput: React.FC<Props> = memo(({ isConnected, textSize, textareaRef, fileInputRef, setUploadedFiles }) => {
    ...
    const onSendMessage = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        if (inputValue.trim() === "" || !socket || !isConnected || isStreaming || isLoading)
            return;
        ...
        const history: AgentInputItem[] = messages.map(message => {
            return message.sender === "bot" ? assistant(message.content) : userRes(message.content)
        })

        console.log("Extracted text is: ", extractedText)
        history.push(userRes(extractedText ? `\nProvided Context from file is ${extractedText}\n\n Answer only in ${selectedLanguage}` : `Answer only in ${selectedLanguage}`))
        ...
        if (!sessionId) {
            ...
            const onSessionStarted = (data: { sessionId: string }) => {
                ...
                socketEvents.model.chatMessage(socket, { sessionId: data.sessionId, history: history, newMessage: inputValue.trim() })
            ...
            socketEvents.model.chatMessage(socket, { sessionId: sessionId, history: history, newMessage: inputValue })
    ...
    const handleLanguageToggle = () => {
        ...
        toast.success(`Language switched to ${selectedLanguage === 'english' ? 'Urdu' : 'English'}`);
    };

    const handleVoiceToggle = () => {
        setIsVoiceRecording(!isVoiceRecording);
        toast.success(isVoiceRecording ? "Voice recording stopped" : "Voice recording started");
    };
    ...
    const handleExportSession = (format: 'pdf' | 'txt' | 'json') => {
        ...
        if (format === 'json') {
            ...
            const text = chatMessages.map(m => `[${m.sender}] ${m.content}`).join('\n\n');
    ...
    return (
        <div className="w-full flex flex-col justify-center items-center mb-1">
            {/* Input Container */}
            <form onSubmit={onSendMessage} className="flex flex-col items-center w-full p-2 bg-neutral border border-border rounded-xl shadow-sm">
                ...
                <div className="flex-1 relative w-full">
                    <textarea
                        ...
                        placeholder={
                            isConnected
                                ? selectedLanguage === 'english' ? "Type your legal question..." : "اپنا قانونی سوال لکھیں..."
                                : "Connecting..."
                        }
                        value={inputValue}
                ...
                <div className="flex justify-between items-center w-full">
                    <div className="flex items-center gap-3">
                        ...
                        <Tooltip>
                            ...
                            <TooltipContent>
                                <p>Switch to {selectedLanguage === 'english' ? 'Urdu' : 'English'}</p>
                            </TooltipContent>
                        </Tooltip>
