...
const modeOptions = ['All', ...Object.values(ConsultationMode)];

const modeIcons: Record<string, any> = {
  [ConsultationMode.VIDEO_CALL]: Video,
  [ConsultationMode.PHONE_CALL]: Phone,
  [ConsultationMode.CHAT]: MessageSquare,
  [ConsultationMode.IN_PERSON]: MapPin,
};
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                {modeOptions.map(opt => {
                  const Icon = opt !== 'All' ? modeIcons[opt] : null;
                  return (
                    <DropdownMenuItem
                      key={opt}
                      onClick={() => setMode(opt)}
                      className={mode === opt ? 'bg-primary/10 font-semibold' : ''}
                    >
                      {Icon && <Icon className="h-4 w-4 mr-2" />}
                      <span className="flex-1">{enumToLabel(opt)}</span>
                      {mode === opt && <span className="text-primary">✓</span>}
                    </DropdownMenuItem>
                  );
                })}
              </DropdownMenuContent>
