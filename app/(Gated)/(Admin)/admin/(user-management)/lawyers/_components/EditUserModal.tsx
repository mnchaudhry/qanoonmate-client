...
export default function EditUserModal({ open, onOpenChange }: EditUserModalProps) {
  ...
  /////////////////////////////////////////////////////// STATES /////////////////////////////////////////////////////////////////
  const [form, setForm] = useState({ firstname: '', lastname: '', email: '', phone: '', role: UserRole.CLIENT as UserRole, accountStatus: AccountStatus.ACTIVE as AccountStatus, releaseChannel: ReleaseChannel.PUBLIC as ReleaseChannel })
  ...
  useEffect(() => {
    if (open && currentUser) {
      setForm({
        ...
        releaseChannel: (currentUser as any).releaseChannel || ReleaseChannel.PUBLIC,
      })
    }
  }, [open, currentUser])
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-surface border border-border max-w-lg">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div>
            <Label className="text-foreground">Release channel</Label>
            <Select value={form.releaseChannel} onValueChange={(v) => setForm({ ...form, releaseChannel: v as ReleaseChannel })}>
              <SelectTrigger className="bg-background border-border text-foreground">
                <SelectValue placeholder="Release channel" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value={ReleaseChannel.ALPHA}>alpha</SelectItem>
                <SelectItem value={ReleaseChannel.BETA}>beta</SelectItem>
                <SelectItem value={ReleaseChannel.PUBLIC}>public</SelectItem>
                <SelectItem value={ReleaseChannel.INTERNAL}>internal</SelectItem>
              </SelectContent>
            </Select>
          </div>
