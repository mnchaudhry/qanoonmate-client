...
import { Bot, ChevronLeft, ChevronRight, Clock, Copy, RotateCcw, ThumbsDown, ThumbsUp, User, ChevronDown, } from "lucide-react";
...
const MessageBox: React.FC<MessageBoxProps> = memo(({ chatViewMode = "card", textSize = 16, messages, onRegenerate }) => {
  ...
  ///////////////////////////////////////////////// VARIABLES ///////////////////////////////////////////////////
  const { quickAction, streamingMessage, isStreaming, isMetadataLoading } = useSelector((state: RootState) => state.aiSession);
  ...
  const MessageItem = React.memo(function MessageItem({ message, index, chatViewMode, textSize, }: MessageItemProps) {
    ...
    const contentToRender = message.isStreaming ? displayedContent : currentResponse.content;
    ...
    return (
      ...
      >
        ...
        {/* Message Actions */}
        <div className={cn("flex flex-col", isModel ? "justify-start items-start" : "justify-end items-end")}>
          {renderMessageActions(message)}
        </div>
      </div>
    );
