...
const CountDownTimer: React.FC = () => {
  ...
  const targetDate = useMemo(() => new Date('12/31/2024 23:59:59'), []);
      ...
      <div>
        <div className="mb-3 flex items-center gap-2">
          {formatNumber(days).map((digit, index) => (
            <div
              key={index}
              className="timer-box relative overflow-hidden rounded-lg"
            >
              <span className="flex h-17.5 min-w-[56px] items-center justify-center rounded-lg bg-black px-3 text-xl font-black leading-[1.35] text-neutral dark:bg-boxdark lg:text-3xl xl:text-[40px]">
                {digit}
              </span>
