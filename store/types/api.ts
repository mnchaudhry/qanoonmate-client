import { ConsultationMode, ConsultationStatus, ConsultationType, UserRole, PaymentMethod, Currency, NotificationDeliveryChannel, NotificationType } from "@/lib/enums";
...
// GET /stats/overview
export interface GetConsultationStatsRequest { dateFrom?: string; dateTo?: string; }
...
export type GetChatSessionResponse = APIResponse<{ session: AIChatSession; messages: AIMessage[] }>;
...
export interface InitiateChatRoomRequest { clientId: string; lawyerId: string; consultationId: string; }
export type InitiateChatRoomResponse = APIResponse<ChatRoom>;

// GET /api/chat/:roomId/messages
export interface GetMessagesRequest { roomId: string; }
export type GetMessagesResponse = APIResponse<Message[]>;

// POST /api/chat/:roomId/messages
export interface SendMessageRequest { content: string; type?: MessageType; }
...
export interface GetUnreadMessageCountRequest { roomId: string; }
export type GetUnreadMessageCountResponse = APIResponse<{ count: number }>;

// DELETE /api/chat/:roomId/messages/:messageId
export interface DeleteMessageRequest { roomId: string; messageId: string; }
...
export interface LegalGuide {
    ...
    status?: 'draft' | 'pending_review' | 'approved' | 'needs_revision' | 'archived';
    isApproved?: boolean;
    verifiedBy?: string | null;
    version?: string;
    lastReviewed?: string;
...
export type StartChatResponse = APIResponse<{ sessionId: string; messages: AIMessage[] }>;

export type UpdateTitleRequest = { sessionId: string; title: string };
export type UpdateTitleResponse = APIResponse<AIChatSession>;

export type ChatMessageRequest = { sessionId: string; message: string; newMessage: string };
