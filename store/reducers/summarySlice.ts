...
import { uploadSummaryDocument, createTextSummary, getUserSummaries, getSummaryStats, getSummaryByIdNew, updateSummaryDetails, retrySummary, exportSummary, deleteSummaryNew } from '../api';
...
// Async thunks
export const uploadDocument = createAsyncThunk('summary/uploadDocument', async (formData: FormData, { rejectWithValue }) => {
    try {
...
export const createSummary = createAsyncThunk('summary/createSummary', async (formData: any, { rejectWithValue }) => {
    try {
...
export const fetchUserSummaries = createAsyncThunk('summary/fetchUserSummaries', async (params: any, { rejectWithValue }) => {
    try {
...
export const fetchSummaryStats = createAsyncThunk('summary/fetchSummaryStats', async (_, { rejectWithValue }) => {
    try {
        const { data } = await getSummaryStats();
        return data?.data;
    } catch (error: any) {
        return rejectWithValue(error.response?.data?.message || 'Failed to fetch summary stats');
    }
}
);

export const fetchSummaryById = createAsyncThunk('summary/fetchSummaryById', async (id: string, { rejectWithValue }) => {
    try {
...
export const updateSummary = createAsyncThunk('summary/updateSummary', async ({ id, formData }: { id: string; formData: { title?: string; metadata?: any; settings?: any } }, { rejectWithValue }) => {
    try {
        const { data } = await updateSummaryDetails(id, formData);
        return data;
    } catch (error: any) {
        return rejectWithValue(error.response?.data?.message || 'Failed to update summary');
    }
}
);

export const retrySummaryProcessing = createAsyncThunk('summary/retrySummary', async (id: string, { rejectWithValue }) => {
    try {
        const { data } = await retrySummary(id);
        return data;
    } catch (error: any) {
        return rejectWithValue(error.response?.data?.message || 'Failed to retry summary');
    }
}
);

export const deleteSummary = createAsyncThunk('summary/deleteSummary', async (id: string, { rejectWithValue }) => {
    try {
...
export const exportSummaryData = createAsyncThunk('summary/exportSummary', async ({ id, format }: { id: string; format?: string }, { rejectWithValue }) => {
    try {
        const { data } = await exportSummary(id, format);
        return data;
    } catch (error: any) {
        return rejectWithValue(error.response?.data?.message || 'Failed to export summary');
    }
}
);
...
const summarySlice = createSlice({
    ...
    reducers: {
        ...
        removeProcessingSummary: (state, action: PayloadAction<string>) => {
            state.processingSummaries = state.processingSummaries.filter(id => id !== action.payload);
        },
        ...
        handleSummaryProgress: (state, action: PayloadAction<SummaryProgress>) => {
            ...
            if (summaryId) {
                ...
                if (state.currentSummary?._id === summaryId) {
                    state.currentSummary.status = progress >= 100 ? 'completed' : 'processing';
                }
            }
            state.progress = progress;
        },
        handleSummaryStream: (state, action: PayloadAction<{ summaryId: string, content: string }>) => {
            if (state.currentSummary && state.currentSummary._id === action.payload.summaryId) {
                state.streamingSummary = action.payload.content;
            }
        },
        handleSummaryComplete: (state, action: PayloadAction<{ summary: Summary }>) => {
            ...
            state.processingSummaries = state.processingSummaries.filter(id => id !== summary._id);
            state.progress = 100;
            state.streamingSummary = '';
        },
        handleSummaryError: (state, action: PayloadAction<{ summaryId: string; error: string }>) => {
            ...
            // Remove from processing list
            state.processingSummaries = state.processingSummaries.filter(id => id !== summaryId);
        },
        handleSummaryAbort: (state, action: PayloadAction<{ summaryId: string }>) => {
    ...
    extraReducers: (builder) => {
        ...
        builder
            ...
            .addCase(updateSummary.fulfilled, (state, action) => {
                ...
                const index = state.summaries.findIndex(s => s._id === updatedSummary._id);
                if (index !== -1) {
