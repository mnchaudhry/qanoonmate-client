...
export const initiateChatRoom = createAsyncThunk('chat/initiateRoom', async (input: { clientId: string; lawyerId: string; consultationId: string }) => {
    try {
        const { data } = await api.initiateChatRoom(input);
        return data?.data;
    } catch (error: any) {
        toast.error(error?.response?.data?.message || 'Failed to initiate chat room');
        throw error;
    }
}
);

export const getUserChatRooms = createAsyncThunk('chat/getUserChatRooms', async () => {
    try {
...
export const getMessages = createAsyncThunk('chat/getMessages', async (roomId: string) => {
    try {
...
export const sendMessage = createAsyncThunk('chat/sendMessage', async ({ roomId, content, type = 'TEXT' }: { roomId: string; content: string; type?: MessageType }) => {
    try {
...
export const markMessagesRead = createAsyncThunk('chat/markMessagesRead', async (roomId: string) => {
    try {
...
export const getUnreadMessageCount = createAsyncThunk('chat/getUnreadMessageCount', async (roomId: string) => {
    try {
...
export const getRecentMessages = createAsyncThunk('chat/getRecentMessages', async (limit: number = 10) => {
    try {
    ...
    reducers: {
        ...
        setTypingStatus: (state, action) => {
            ...
            if (isTyping) {
                ...
                const existingIndex = state.roomStates[roomId].typingUsers.indexOf(userId);
                if (existingIndex === -1) {
                ...
                const globalIndex = state.typingUsers.findIndex(t => t.userId === userId && t.roomId === roomId);
                if (globalIndex === -1) {
                ...
                // Remove typing user
                state.roomStates[roomId].typingUsers = state.roomStates[roomId].typingUsers.filter(id => id !== userId);
                console.log('Removed user from typing list:', userId);

                // Remove from global typing users
                state.typingUsers = state.typingUsers.filter(t => !(t.userId === userId && t.roomId === roomId));
            }

            console.log('Current typing users in room:', state.roomStates[roomId].typingUsers);
        },
        ...
        setOnlineStatus: (state, action) => {
            ...
            if (isOnline) {
                // Add online user
                const existingIndex = state.roomStates[roomId].onlineUsers.indexOf(userId);
                if (existingIndex === -1) {
                    state.roomStates[roomId].onlineUsers.push(userId);
                }

                // Add to global online users
                const globalIndex = state.onlineUsers.findIndex(o => o.userId === userId && o.roomId === roomId);
                if (globalIndex === -1) {
                ...
                // Remove online user
                state.roomStates[roomId].onlineUsers = state.roomStates[roomId].onlineUsers.filter(id => id !== userId);

                // Remove from global online users
                state.onlineUsers = state.onlineUsers.filter(o => !(o.userId === userId && o.roomId === roomId));
            }
        },
        ...
        builder
            ...
            .addCase(getMessages.fulfilled, (state, action) => {
                ...
                if (action.payload?.messages) {
                    ...
                    state.messages[roomId]?.push(...action.payload.messages);
                }
            })
            ...
            .addCase(sendMessage.fulfilled, (state, action) => {
                ...
                // Update room's last message
                const room = state.rooms.find(r => r._id === action.payload?.chatRoomId);
                ...
                Object.values(state.messages).forEach(messages => {
                    messages.forEach(message => {
                        if (message.chatRoomId === roomId) {
                            ...
                            const currentUser = state.currentRoom?.participants?.find(p => p._id !== message.sender._id);
                            if (currentUser && !message.readBy.some(reader => reader._id === currentUser._id)) {
            ...
            .addCase(deleteMessage.fulfilled, (state, action) => {
                ...
                Object.keys(state.messages).forEach(roomId => {
                    state.messages[roomId] = state.messages[roomId]?.filter(message => message._id !== messageId) || [];
                });
            })
            ...
            .addCase(getRecentMessages.rejected, (state, action) => {
                state.isRecentLoading = false;
                state.recentError = action.error.message || 'Failed to fetch recent messages';
            });
    },
});
